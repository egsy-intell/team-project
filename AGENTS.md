...
- When you commit work you paired on with a human, add a
  `Co-Authored-By:` trailer identifying yourself (e.g. `Co-Authored-By:
  Claude Sonnet 5 <noreply@anthropic.com>` — use whatever line your own
  tool's convention is). It's not required for `docs/ai/`'s disclosure
  tooling to work (see `docs/ai/skill/README.md` — commit attribution
  there is by author identity and timestamp, not by this trailer, since
  not everyone tags commits this way), but it makes the git history
  itself a queryable record of AI involvement, which is worth doing
  regardless.
...
- `.github/workflows/autofix-lint.yml` — on PRs, runs `ruff check --fix`
  ...
  conflict below.
- `docs/ai/` — per-teammate AI Tool Use Policy disclosures (checked into
  git, unlike the CI-generated `docs/notebooks/`; both publish to
  `gh-pages` together, see `publish.yml`). `docs/ai/<person>.html` and
  `docs/ai/logs/<person>/` are generated by `scripts/toolkit.py
  ai-disclosure <person>` from a manifest — never hand-edit those two
  outputs directly. `docs/ai/skill/` has the full runbook
  (`README.md`), the policy text (`POLICY.md`), and a worked example
  manifest; it's written to be followed by any AI coding agent, not
  Claude specifically, since not every teammate pairs with the same
  tool.
...
uv run python scripts/toolkit.py ai-disclosure <person>  # render docs/ai/<person>.html + logs index
```
